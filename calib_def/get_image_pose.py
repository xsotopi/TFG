...
ADD_IMAGES = 20                  # cuántas capturas NUEVAS quieres
REQ_W, REQ_H = 1280, 800
# ───────────────────────────────────────────────

IMG_DIR  = SAVE_DIR / "images"; IMG_DIR.mkdir(parents=True, exist_ok=True)
POSE_DIR = SAVE_DIR / "poses";  POSE_DIR.mkdir(parents=True, exist_ok=True)

# ―― averiguar cuántas fotos ya hay ―――――――――――――――――――――――
already = sorted(IMG_DIR.glob("img_*.png"))
start_idx = 0
if already:
    m = re.search(r"img_(\d+)_", already[-1].stem)
    if m:      # último índice usado
...
def get_tcp_pose():
    with socket.create_connection((UR_IP, RT_PORT), timeout=0.5) as s:
        hdr = s.recv(4); (length,) = struct.unpack(">I", hdr)
...
print(f"🟢 Capturadas {captured} nuevas imágenes. Total ahora: {start_idx+captured}")
