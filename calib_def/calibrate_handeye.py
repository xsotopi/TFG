#!/usr/bin/env python3
"""
calibrate_handeye.py – single‑pass 0.5 σ pruning + save kept‑frame list
======================================================================
* Same behaviour as before **plus**:
  • Saves the list of image stems that survived pruning in `hand_eye.json`
    under key ``"frames_kept"``.
* Validation can now load that list and evaluate only the inlier frames.
"""
...
BOARD_SIZE = (8,7); SQUARE_LEN_M = 0.015
SIGMA_THRS = 0.5; MIN_FRAMES = 10
METHOD     = cv2.CALIB_HAND_EYE_TSAI

IMG_DIR = DATASET_DIR/"images"; POSE_DIR=DATASET_DIR/"poses"
RES_DIR = DATASET_DIR/"calib_results"; RES_DIR.mkdir(exist_ok=True)

# helpers --------------------------------------------------------------------

def skew(w):
    x,y,z=w; return np.array([[0,-z,y],[z,0,-x],[-y,x,0]],dtype=w.dtype)
...
def se3_log_norm(T):
    ...
    if th<1e-9:
        ...
        A=math.sin(th)/th; B=(1-math.cos(th))/th**2
        Vinv=np.eye(3)-0.5*skew(w)+(1/th**2)*(1-A/(2*B))*skew(w)@skew(w)
    v=Vinv@t; return float(np.linalg.norm(np.hstack((w,v))))

def obj_corners():
    g=np.mgrid[0:BOARD_SIZE[0],0:BOARD_SIZE[1]].T.reshape(-1,2).astype(np.float32)
    pts=np.zeros((len(g),3),np.float32); pts[:,:2]=g*SQUARE_LEN_M; return pts

def rt_to_T(r,t):
    R,_=cv2.Rodrigues(r); T=np.eye(4); T[:3,:3]=R; T[:3,3]=t.ravel(); return T

# load -----------------------------------------------------------------------
print("▶ scanning", IMG_DIR)
paths=sorted(IMG_DIR.glob("img_*.png"))
if not paths: sys.exit("✗ no images found")
objp=obj_corners(); objpts=[]; imgpts=[]; rv_r=[]; tv_r=[]; keep_names=[]
for p in tqdm.tqdm(paths,desc="detect"):
    im=cv2.imread(str(p)); gray=cv2.cvtColor(im,cv2.COLOR_BGR2GRAY)
    ...
    objpts.append(objp); imgpts.append(c); keep_names.append(p.stem)
    tcp=np.array(json.loads((POSE_DIR/(p.stem.replace('img','pose')+'.json')).read_text())['pose_tcp'],np.float64)
    rv_r.append(tcp[3:].reshape(3,1)); tv_r.append(tcp[:3,None])

if len(objpts)<MIN_FRAMES: sys.exit("✗ not enough detections")
...
if mask.sum()<MIN_FRAMES: sys.exit("✗ would fall below MIN_FRAMES – abort")
...
# save -----------------------------------------------------------------------
np.save(RES_DIR/"K.npy",K); np.save(RES_DIR/"distCoeffs.npy",dist); np.save(RES_DIR/"cam_to_tcp.npy",X)
json.dump({
