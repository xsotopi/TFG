...
# ---------- calibración ----------
CALIB_DIR = "camera"
K_data     = np.load(os.path.join(CALIB_DIR, "intrinsics.npz"))
K, dist    = K_data["K"], K_data["dist"]
T_cam2tcp  = np.load(os.path.join(CALIB_DIR, "T_cam2tcp.npy"))
print("[INFO] Calibración OK")

T_tcp_cam = np.linalg.inv(T_cam2tcp)      # hazlo UNA sola vez, fuera del bucle


# ---------- modelos ----------
whisper_model = whisper.load_model("small"); print("Whisper OK")
yolo_model    = YOLO("yolov8n.pt");          print("YOLO OK")
...
print("SLM OK on", device)

# ---------- constantes ----------
UPLOAD_FOLDER = "app/uploads"; AUDIO_EXT = ".mp3"
    ...
    while True:
        ...
        if current_intent == "pick" and current_target_object and show_detections:
            ...
            if detections:
                ...
                if depth_frame is not None and not pose_sent:

                    pose = compute_pose(bbox, depth_frame, color_frame.shape[:2], K, dist, T_tcp_cam)
                    if pose:
